...
posterior :
  ...
  Shared $ GaussianProcess features
...
data ConjugateGPRegression : (0 features : Shape) -> Type where
  ...
  MkConjugateGPR :
    {p : _} ->
    (gpFromHyperparameters : Tensor [p] F64 -> Shared $ GaussianProcess features) ->
...
fit (MkConjugateGPR {p} mkPrior gpParams noise) optimizer (MkDataset x y) =
  let objective : Tensor [S p] F64 -> Tensor [] F64
      ...
      mkPosterior : Tensor [p] F64 -> Shared $ GaussianProcess features
