...
Cache : Type
Cache = SortedMap Bits64 (List ((shape ** Expr shape), XlaOp))

Computation : Type -> Type
Computation = StateT (XlaBuilder, Cache) IO

cached : {shape : _} -> Expr shape -> Computation XlaOp -> Computation XlaOp
cached graph xs = let graphHash = hash graph in do
  (_, cache) <- get
  case lookup graphHash cache of
    Just candidates => case find (\(graph', _) => exprCompare graph graph') candidates of
      Just (_, op) => pure op
  ...
  runOp : {shape : _} ->
          ...
          Bits64 ->
          Expr shape ->
          List ((shape' ** Expr shape'), XlaOp) ->
          Computation XlaOp
  runOp xs key graph graphOps = do
    op <- xs
    (builder, cache) <- get
    put (builder, insert key (((_ ** graph), op) :: graphOps) cache)
    pure op

  exprCompare : {shape : _} -> Expr shape -> (shape' ** Expr shape') -> Bool
  exprCompare {shape} expr (shape' ** expr') = case decEq shape shape' of
    Yes eq => expr == rewrite eq in expr'
    _ => False
...
-- we may not need {shape : _} -> if we update idris
covering
enqueue : {shape : _} -> Expr shape -> Computation XlaOp
enqueue e@(FromLiteral {shape, dtype} lit) = cached e $ do
  (builder, _) <- get
  literal <- write {dtype} lit 
  constantLiteral builder literal
enqueue e@(Parameter {shape, dtype} position name) = cached e $ do
  (builder, _) <- get
...
enqueue e@(Slice starts stops expr) = cached e $ slice !(enqueue expr) starts stops (replicate 1 (length starts))
...
enqueue e@(Identity {n, dtype}) = cached e $ let n = cast n in do
  (builder, _) <- get
  identityMatrix {dtype} builder n n
enqueue e@(Broadcast {from, to, dtype} expr) = cached e $
...
enqueue e@(Reduce (p0, p1, exprf) neutral axes expr) = cached e $ do
...
export
toString : {shape : _} -> Expr shape -> String
...
export
run : PrimitiveRW dtype a => {shape : _} -> Expr shape -> Literal shape a
