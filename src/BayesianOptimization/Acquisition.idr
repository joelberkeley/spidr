...
public export 0
Empiric : Distribution targets marginal =>
  (0 features : Shape) -> {0 model : Type} -> (0 out : Type) -> Type
Empiric {targets} {marginal} features out = ProbabilisticModel features targets marginal model =>
  {s : _} -> Data {samples=S s} features targets -> model -> out
...
export
expectedImprovement : ProbabilisticModel features [1] Gaussian m =>
                      m -> (best : Tensor [] Double) -> Acquisition 1 features
...
export
expectedImprovementByModel : Empiric features {targets=[1]} {marginal=Gaussian} $ Acquisition 1 features
expectedImprovementByModel (query_points, _) model at =
  let best = squeeze {from=[1]} $ reduce_min 0 $ mean $ marginalise model query_points
...
probabilityOfFeasibility limit _ model at = cdf (marginalise model at) $ broadcast {to=[1, 1]} limit
...
negativeLowerConfidenceBound : (beta : Double) ->
  Either ValueError $ Empiric features {targets=[1]} {marginal=Gaussian} $ Acquisition 1 features
negativeLowerConfidenceBound beta =
  ...
  then Left $ MkValueError $ "beta should be greater than or equal to zero, got " ++ show beta
    ...
    impl : Empiric features {targets=[1]} {marginal=Gaussian} $ Acquisition 1 features
    impl _ model x = let marginal = marginalise model x
                         mean = squeeze {from=[1, 1]} {to=[]} $ mean marginal
                         variance = squeeze {from=[1, 1]} {to=[]} $ variance marginal
...
expectedConstrainedImprovement : (limit : Tensor [] Double) ->
  Empiric features {targets=[1]} {marginal=Gaussian} $
