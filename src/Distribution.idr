...
||| same shape.
public export
interface TwoCentralMoment (0 dist : Shape -> Nat -> Type) where
  ||| The mean of the distribution.
  mean : dist event dim -> Tensor (dim :: event) Double

  ||| The covariance, or correlation, between sub-events.
  covariance : dist event dim -> Tensor (dim :: dim :: event) Double

||| The variance of a single random variable.
export
variance : TwoCentralMoment dist => dist event 1 -> Tensor (1 :: event) Double
variance dist = squeeze {from=(1 :: 1 :: event)} $ covariance dist
...
public export
data Gaussian : (event : Shape) -> (dim : Nat) -> Type where
  ||| @mean The Gaussian mean.
  ||| @covariance The Gaussian covariance.
  MkGaussian : {d : Nat} -> (mean : Tensor (S d :: event) Double) ->
               (covariance : Tensor (S d :: S d :: event) Double) ->
               Gaussian event (S d)

export
TwoCentralMoment Gaussian where
