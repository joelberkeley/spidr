...
test_elementwise_equality = do
    ...
    where
        compareScalars : Primitive dtype => Prelude.Eq ty => PrimitiveRW dtype ty => Primitive.Eq dtype => ty -> ty -> IO ()
...
test_elementwise_inequality = do
    ...
    where
        compareScalars : Primitive dtype => Primitive.Eq dtype => Prelude.Eq ty => PrimitiveRW dtype ty =>
                         ty -> ty -> IO ()
        compareScalars l r =
...
test_comparison = do
    ...
    where
        compareScalars : Primitive.Ord dtype => Prelude.Ord ty => PrimitiveRW dtype ty => Primitive dtype => ty -> ty -> IO ()
        compareScalars l r = do
            let l' = const {dtype} l
                r' = const {dtype} r
