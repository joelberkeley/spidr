...
extern "C" {
    Tensor* absOp(Tensor& operand, ShapedType& resultType) {
        auto res = mlir::stablehlo::absOp(
            reinterpret_cast<mlir::stablehlo::Tensor&>(operand),
            reinterpret_cast<mlir::ShapedType&>(resultType)
        );
        // is this the best way to heap-allocate this?
        return reinterpret_cast<Tensor*>(new mlir::stablehlo::Tensor(res));
    }

    Tensor* addOp(Tensor& lhs, Tensor& rhs, ShapedType& resultType) {
        auto res = mlir::stablehlo::addOp(
            reinterpret_cast<mlir::stablehlo::Tensor&>(lhs),
            reinterpret_cast<mlir::stablehlo::Tensor&>(rhs),
            reinterpret_cast<mlir::ShapedType&>(resultType)
        );
        return reinterpret_cast<Tensor*>(new mlir::stablehlo::Tensor(res));
    }
