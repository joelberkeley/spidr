...
-- tensorlist/tensorevallist has horrid ergonomics - either merge the two tensorlists, or change the syntax of one
...
|||
||| **Note:** Values tagged in the same scope as `while` cannot then be used within the `condition`
||| or `body`. This is due to an [issue in XLA](https://github.com/openxla/xla/issues/14299).
|||
...
while condition body initials = do
  ...
  asTensorList =<< tag (While condition body (Tuple $ exprs initials))
